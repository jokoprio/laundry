...
@section('title', isset($user) ? 'Edit Admin User' : 'Tambah Admin User')
@section('header', isset($user) ? 'Edit Admin User' : 'Tambah Admin User')
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h3 class="text-xl font-extrabold text-white">
                            {{ isset($user) ? 'Edit Admin User' : 'Tambah Admin User Baru' }}
                        </h3>
                        <p class="text-blue-100 text-sm mt-1">Isi form di bawah dengan lengkap</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="role" 
                                id="role"
                                class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all @error('role') border-red-300 @enderror"
                                required>
                ...
                <div id="permissions-section" style="display: none;">
                    <div class="border-t border-slate-200 pt-6">
                        <h4 class="text-lg font-bold text-slate-800 mb-4">
                            <i class="fas fa-shield-halved text-blue-600 mr-2"></i>Permissions
                        </h4>
                        <p class="text-sm text-slate-600 mb-4">Pilih permissions yang akan diberikan kepada user ini</p>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach($availablePermissions as $category => $permissions)
                                <div class="bg-slate-50 rounded-xl p-4">
                                    <h5 class="font-bold text-slate-700 mb-3 text-sm">{{ $category }}</h5>
                                    <div class="space-y-2">
